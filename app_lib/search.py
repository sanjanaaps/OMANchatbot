...
import logging
from app_lib.db import get_documents_by_department
...
def search_documents(query: str, department: str, top_k: int = 10) -> List[Dict]:
    ...
    """
    try:
        # Get documents for the department via MongoDB accessor
        documents = get_documents_by_department(department)
        # Ensure an `_id` field exists for downstream usage
        for d in documents:
